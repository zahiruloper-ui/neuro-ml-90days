...
df = pd.DataFrame(
    {
        "id": [1, 2, 3, 4, 5],
        "city": ["Vancouver", "Vancouver", None, "Burnaby", "Burnaby"],
        "age": [29, np.nan, 41, 33, np.nan],
        "spend": [120.5, 0.0, np.nan, 75.0, 10.0],
        "member": [True, True, False, None, False],
    }
)

# Clean (same idea as Task 2)
df = df.fillna(
    {"city": "Unknown", "age": df["age"].median(), "spend": 0.0, "member": False}
)
df["id"] = df["id"].astype("string")
df["member"] = df["member"].astype("boolean")

print("INFO:")
df.info()  # prints column dtypes + non-null counts 

print("\nDESCRIBE (numeric):")
print(df.describe())  # summary stats for numeric columns [web:54]

print("\nDESCRIBE (include='all'):")
print(df.describe(include="all"))

print("\nVALUE_COUNTS city (dropna=False):")
print(df["city"].value_counts(dropna=False))  # include NaN bucket if present 

print("\nVALUE_COUNTS member (dropna=False):")
print(df["member"].value_counts(dropna=False))  # include NaN bucket if present 
print(df.dtypes)
print(df["member"].isna().sum())
